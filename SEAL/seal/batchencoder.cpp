...
namespace seal
{
    BatchEncoder::BatchEncoder(const std::shared_ptr<SEALContext> &context) : 
        context_(context)
    {
        // Verify parameters
        if (!context_)
        ...
        Plaintext &destination)
    {
        ...
        {
            // Validate the i-th input
            if (static_cast<uint64_t>(llabs(v)) > plain_modulus_div_two)
            {
                throw invalid_argument("input value is larger than plain_modulus");
            }
        }
...
#endif
    void BatchEncoder::encode(Plaintext &plain, const MemoryPoolHandle &pool)
    {
        ...
        const MemoryPoolHandle &pool)
    {
        ...
        if(destination.size() > static_cast<index_type>(numeric_limits<int>::max()) || 
            destination.size() != static_cast<index_type>(slots_))
        {
...
#endif
    void BatchEncoder::decode(Plaintext &plain, const MemoryPoolHandle &pool)
    {
        if (plain.is_ntt_form())
        {
            throw invalid_argument("plain cannot be in NTT form");
