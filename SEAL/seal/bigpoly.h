...
namespace seal
{
    ...
    */
    ...
    public:
        ...
        */
        BigPoly(std::size_t coeff_count, int coeff_bit_count);
        ...
        */
        ...
        @throws std::invalid_argument if hex_poly does not adhere to the expected format
        */
        BigPoly(std::size_t coeff_count, int coeff_bit_count, const std::string &hex_poly);
        ...
        @throws std::invalid_argument if coeff_bit_count is negative or
        value is null and coeff_count and coeff_bit_count are positive 
        */
        BigPoly(std::size_t coeff_count, int coeff_bit_count, std::uint64_t *value);
        ...
        */
        inline std::uint64_t *data(std::size_t coeff_index)
        {
        ...
        */
        inline const std::uint64_t *data(std::size_t coeff_index) const
        {
            if (coeff_count_ == 0 || coeff_bit_count_ == 0)
            {
                return nullptr;
        ...
        */
        inline std::size_t significant_coeff_count() const
        {
            if (coeff_count_ == 0 || coeff_bit_count_ == 0)
            {
                return 0;
        ...
        */
        inline bool operator ==(const BigPoly &compare) const
        {
            ...
            std::size_t compare_sig_coeff_count = compare.significant_coeff_count();
            if (sig_coeff_count != compare_sig_coeff_count)
            {
                return false;
            }
            for (std::size_t i = 0; i < sig_coeff_count; i++)
            {
        ...
        */
        inline const BigUInt &operator [](std::size_t coeff_index) const
        {
            if (coeff_index >= coeff_count_)
            {
        ...
        @see BigUInt for operations that can be performed on the coefficients.
        */
        inline BigUInt &operator [](std::size_t coeff_index)
        {
            if (coeff_index >= coeff_count_)
            {
                throw std::out_of_range("coeff_index must be within [0, coefficient count)");
        ...
        */
        inline void set_zero(std::size_t start_coeff, std::size_t length)
        {
            ...
            if (start_coeff + length > coeff_count_)
            {
        ...
        @param[in] coeff_bit_count The bit width of each coefficient
        @throws std::invalid_argument if either coeff_count or coeff_bit_count is negative
        @throws std::invalid_argument if polynomial size is too large
        @throws std::logic_error if the BigPoly is an alias
        */
        void resize(std::size_t coeff_count, int coeff_bit_count);
        ...
        @param[in] coeff_bit_count The bit width of each coefficient
        @param[in] value The backing array to use
        @throws std::invalid_argument if coeff_bit_count is negative or value is null
        @see BigPoly for a more detailed description of aliased BigPolys.
        */
        void alias(std::size_t coeff_count, int coeff_bit_count, std::uint64_t *value);
