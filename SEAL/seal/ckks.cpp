...
namespace seal
{
    ...
    CKKSEncoder::CKKSEncoder(const shared_ptr<SEALContext> &context) : 
        context_(context)
    {
        // Verify parameters
        if (!context_)
        ...
        complex<double> psi{ cos((2 * PI_) / static_cast<double>(m)), sin((2 * PI_) / static_cast<double>(m)) };
    ...
    void CKKSEncoder::encode_internal(double value, 
        ...
        const MemoryPoolHandle &pool)
    {
        ...
        if (coeff_bit_count >= context_data.total_coeff_modulus().significant_bit_count())
        {
