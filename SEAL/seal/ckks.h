...
namespace seal
{
    ...
    */
    ...
    public:
        ...
        */
        CKKSEncoder(const std::shared_ptr<SEALContext> &context);
        ...
        */
        ...
        {
            encode_internal(values, parms_id, scale, destination, pool);
        }
        ...
        */
        ...
        inline void encode(const std::vector<T> &values,
            ...
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            encode(values, context_->first_parms_id(), scale, destination, pool);
        }
        ...
        */
        inline void encode(double value, parms_id_type parms_id,
            ...
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            encode_internal(value, parms_id, scale, destination, pool);
        }
        ...
        */
        inline void encode(double value,
            ...
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            encode(value, context_->first_parms_id(), scale, destination, pool);
        }
        ...
        */
        template<typename T,
            ...
            inline void decode(const Plaintext &plain, std::vector<T> &destination,
                const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            decode_internal(plain, destination, pool);
        }
    ...
    private:
                ...
                const MemoryPoolHandle &pool)
        {
            ...
            if (values.size() > static_cast<std::size_t>(slots_))
            {
            ...
            else if (max_coeff_bit_count <= 128)
            {
                for (std::size_t i = 0; i < n; i++)
                {
                    ...
                    else
                    {
                        for (std::size_t j = 0; j < coeff_mod_count; j++)
                        {
                            destination[i + j * coeff_count] = util::barrett_reduce_128(
            ...
            else
            {
                ...
                for (std::size_t i = 0; i < n; i++)
                {
                    ...
                    if (is_negative)
                    {
                        ...
                        {
                            destination[i + (j * coeff_count)] = util::negate_uint_mod(
                                decomp_coeffu[j], coeff_modulus[j]);
                        }
                    }
                    else
                    {
            ...
            for (std::size_t i = 0; i < coeff_mod_count; i++)
            {
                util::ntt_negacyclic_harvey(
                    destination.data() + (i * coeff_count), coeff_small_ntt_tables[i]);
            }
            ...
            const MemoryPoolHandle &pool)
        {
            ...
            std::size_t coeff_mod_count = coeff_modulus.size();
            std::size_t coeff_count = parms.poly_modulus_degree();
            std::size_t rns_poly_uint64_count = coeff_count * coeff_mod_count;

            auto &coeff_small_ntt_tables = context_data_ptr->small_ntt_tables();

            // Check that scale is positive and not too large
            if (plain.scale() <= 0 || (static_cast<int>(log2(plain.scale())) >=
                context_data_ptr->total_coeff_modulus().significant_bit_count()))
            {
                throw std::invalid_argument("scale out of bounds");
            }
            ...
            // destination mod q
            auto wide_tmp_dest(util::allocate_zero_uint(coeff_mod_count * n, pool));

            // Transform each polynomial from NTT domain
            for (std::size_t i = 0; i < coeff_mod_count; i++)
            {
                util::inverse_ntt_negacyclic_harvey(
                    plain_copy.get() + (i * coeff_count), coeff_small_ntt_tables[i]);
            }

            auto res = util::allocate<std::complex<double>>(n, pool);
            ...
            for (std::size_t i = 0; i < n; i++)
            {
                for (std::size_t j = 0; j < coeff_mod_count; j++)
                {
                    ...
                    util::modulo_uint(temp.get(), coeff_mod_count,
                        decryption_modulus, temp.get(), pool);
                    util::add_uint_uint_mod(temp.get(),
                        wide_tmp_dest.get() + (i * coeff_mod_count),
                        decryption_modulus.get(), coeff_mod_count,
                        wide_tmp_dest.get() + (i * coeff_mod_count));
                }

                // Reduction modulo total_coeff_modulus
                util::modulo_uint_inplace(
                    wide_tmp_dest.get() + (i * coeff_mod_count),
                    coeff_mod_count, decryption_modulus, pool);
                ...
                {
                    for (std::size_t j = 0; j < decryption_modulus.uint64_count(); j++)
                    {
                        double diff = 0.0;
                        if (wide_tmp_dest[i * coeff_mod_count + j] > decryption_modulus.get()[j])
                        {
                            diff = static_cast<double>(wide_tmp_dest[i * coeff_mod_count + j]
                                - decryption_modulus.get()[j]);
                        }
                        else
                        {
                            diff = -static_cast<double>(decryption_modulus.get()[j]
                                - wide_tmp_dest[i * coeff_mod_count + j]);
                        }
                        res_accum += static_cast<double>(diff) * pow(two_pow_64, j);
                    }
                }
                else
                {
            ...
            const MemoryPoolHandle &pool)
        {
            encode_internal(std::vector<std::complex<double>>(1, value),
                parms_id, scale, destination, pool);
        }
