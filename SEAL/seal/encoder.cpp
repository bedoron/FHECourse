...
namespace seal
{
    ...
    {
        size_t encode_coeff_count = static_cast<size_t>(get_significant_bit_count(value));
    ...
    void BinaryEncoder::encode(int64_t value, Plaintext &destination)
    {
        if (value < 0)
        {
            ...
            size_t encode_coeff_count = static_cast<size_t>(get_significant_bit_count(pos_value));
            destination.resize(encode_coeff_count);
            destination.set_zero();
    ...
    {
        size_t encode_coeff_count = static_cast<size_t>(value.significant_bit_count());
        destination.resize(encode_coeff_count);
        destination.set_zero();
        
        size_t coeff_index = 0;
        size_t coeff_count = static_cast<size_t>(value.significant_bit_count());
        size_t coeff_uint64_count = value.uint64_count();
        while (coeff_index < coeff_count)
        {
            if (is_bit_set_uint(value.data(), coeff_uint64_count, static_cast<int>(coeff_index)))
            {
                destination[coeff_index] = 1;
            }
            coeff_index++;
    ...
    BigUInt BinaryEncoder::decode_biguint(const Plaintext &plain)
    {
        ...
        int result_bit_capacity = static_cast<int>(result_uint64_count) * bits_per_uint64;
        ...
        for (size_t bit_index = plain.significant_coeff_count(); bit_index--; )
        {
            ...
            if (is_bit_set_uint(result, result_uint64_count, result_bit_capacity - 1))
            {
                ...
                result_uint64_count++;
                result_bit_capacity = static_cast<int>(result_uint64_count) * bits_per_uint64;
            ...
            if (result_is_negative == coeff_is_negative)
            {
                ...
                if (add_uint_uint64(result, *pos_pointer, result_uint64_count, result))
                {
                    ...
                    int carry_bit_index = static_cast<int>(result_uint64_count) * bits_per_uint64;
                    result_uint64_count++;
                    result_bit_capacity = static_cast<int>(result_uint64_count) * bits_per_uint64;
                    resultint.resize(result_bit_capacity);
                    result = resultint.data();
    ...
    void BalancedEncoder::encode(const BigUInt &value, Plaintext &destination)
    {
        ...
        size_t encode_uint64_count = static_cast<size_t>(
            divide_round_up(static_cast<int>(encode_coeff_count), bits_per_uint64));
    ...
    int64_t BalancedEncoder::decode_int64(const Plaintext &plain)
    {
        ...
        for (size_t bit_index = plain.significant_coeff_count(); bit_index--; )
        {
            ...
            int64_t next_result = result * static_cast<int64_t>(base_);
    ...
    BinaryFractionalEncoder::BinaryFractionalEncoder(const SmallModulus &plain_modulus, 
        ...
        poly_modulus_degree_(poly_modulus_degree)
    {
        ...
        if (integer_coeff_count_ + fraction_coeff_count_ > poly_modulus_degree_)
        {
    ...
    Plaintext BinaryFractionalEncoder::encode(double value)
    {
        ...
        left_shift_uint(encoded_fract.data(), bits_per_uint64 * 
            static_cast<int>(poly_modulus_degree_ - fraction_coeff_count_),
            poly_modulus_degree_, encoded_fract.data());
    ...
    BalancedFractionalEncoder::BalancedFractionalEncoder(const SmallModulus &plain_modulus, 
        size_t poly_modulus_degree, size_t integer_coeff_count, size_t fraction_coeff_count, 
        uint64_t base) : 
