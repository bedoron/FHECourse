...
namespace seal
{
    Encryptor::Encryptor(const shared_ptr<SEALContext> &context, const PublicKey &public_key) :
        context_(context)
    {
        // Verify parameters
        if (!context_)
        ...
        const MemoryPoolHandle &pool)
    {
        ...
        case scheme_type::BFV:
            bfv_encrypt(plain, destination, pool);
            return;

        case scheme_type::CKKS:
            ckks_encrypt(plain, destination, pool);
            return;
    ...
    void Encryptor::bfv_encrypt(const Plaintext &plain, Ciphertext &destination,
        const MemoryPoolHandle &pool)
    {
        ...
        */
        ...
        {
            ntt_double_multiply_poly_nttpoly(u.get() + (i * coeff_count), 
                ...
                destination.data(1) + (i * coeff_count), pool);
        }
    ...
    void Encryptor::ckks_encrypt(const Plaintext &plain, Ciphertext &destination,
        const MemoryPoolHandle &pool)
    {
        if (!plain.is_ntt_form())
        {
        ...
        if (plain.coeff_count() > coeff_count * coeff_mod_count)
        {
            throw invalid_argument("plain is not valid for encryption parameters");
        }
#endif
        ...
        destination.scale_ = plain.scale();
