...
namespace seal
{
    ...
    Evaluator::Evaluator(const std::shared_ptr<SEALContext> &context) :
        context_(context)
    {
        // Verify parameters
        if (!context_)
    ...
    void Evaluator::multiply_inplace(Ciphertext &encrypted1, 
        const Ciphertext &encrypted2, const MemoryPoolHandle &pool)
    {
        // Verify parameters.
        auto context_data_ptr = context_->context_data(encrypted1.parms_id());
        if (!context_data_ptr)
        ...
        const MemoryPoolHandle &pool)
    {
        ...
        size_t bsk_mtilde_count = bsk_base_mod_count + 1;
        ...
        size_t dest_count = encrypted1_size + encrypted2_size - 1;
    ...
    void Evaluator::ckks_multiply(Ciphertext &encrypted1, const Ciphertext &encrypted2,
        const MemoryPoolHandle &pool)
    {
        if (!(encrypted1.is_ntt_form() && encrypted2.is_ntt_form()))
        {
    ...
    void Evaluator::square_inplace(Ciphertext &encrypted, const MemoryPoolHandle &pool)
    {
        // Verify parameters.
        auto context_data_ptr = context_->context_data(encrypted.parms_id());
        if (!context_data_ptr)
        ...
        {
        case scheme_type::BFV:
            bfv_square(encrypted, pool);
            return;

        case scheme_type::CKKS:
            ckks_square(encrypted, pool);
            return;
    ...
    void Evaluator::bfv_square(Ciphertext &encrypted, const MemoryPoolHandle &pool)
    {
        ...
        if (encrypted_size != 2)
        {
            bfv_multiply(encrypted, encrypted, pool);
            return;
        }
        ...
        size_t dest_count = (encrypted_size << 1) - 1;
    ...
    void Evaluator::ckks_square(Ciphertext &encrypted, const MemoryPoolHandle &pool)
    {
        if (!encrypted.is_ntt_form())
        {
    ...
    void Evaluator::mod_switch_scale_to_next(const Ciphertext &encrypted, Ciphertext &destination,
        const MemoryPoolHandle &pool)
    {
        // Verify parameters.
        auto context_data_ptr = context_->context_data(encrypted.parms_id());
        ...
        destination.is_ntt_form_ = false;
        ...
        Ciphertext &destination)
    {
        ...
        if (encrypted.scale() <= 0 || (static_cast<int>(log2(encrypted.scale())) >=
            next_context_data.total_coeff_modulus().significant_bit_count()))
        {
            throw invalid_argument("scale out of bounds");
        }
    ...
    void Evaluator::mod_switch_drop_to_next(Plaintext &plain)
    {
        ...
        plain.resize(next_coeff_mod_count * coeff_count);
        ...
        Ciphertext &destination, const MemoryPoolHandle &pool)
    {
        ...
        case scheme_type::BFV:
            ...
            mod_switch_scale_to_next(encrypted, destination, pool);
            return;
    ...
    void Evaluator::mod_switch_to_inplace(Ciphertext &encrypted, parms_id_type parms_id,
        const MemoryPoolHandle &pool)
    {
        // Verify parameters.
        auto context_data_ptr = context_->context_data(encrypted.parms_id());
        ...
        const RelinKeys &relin_keys, const MemoryPoolHandle &pool)
    {
        ...
        multiply_many(exp_vector, relin_keys, encrypted, pool);
    }
    ...
    void Evaluator::add_plain_inplace(Ciphertext &encrypted, const Plaintext &plain)
    {
        ...
        case scheme_type::BFV:
        {
            ...
            if (!are_poly_coefficients_less_than(plain.data(),
                plain.coeff_count(), 1, parms.plain_modulus().data(), 1))
            {
                throw invalid_argument("plain is not valid for encryption parameters");
            }
#endif
        ...
        case scheme_type::CKKS:
        {
            for (size_t j = 0; j < coeff_mod_count; j++)
            {
                ...
                if (!are_poly_coefficients_less_than(plain.data(j * coeff_count),
                    coeff_count, 1, coeff_modulus[j].data(), 1))
                {
                    throw invalid_argument("plain is not valid for encryption parameters");
                }
#endif
    ...
    void Evaluator::multiply_plain_inplace(Ciphertext &encrypted, const Plaintext &plain,
        const MemoryPoolHandle &pool)
    {
        // Verify parameters.
    ...
    void Evaluator::multiply_plain_normal(Ciphertext &encrypted, const Plaintext &plain,
        const MemoryPoolHandle &pool)
    {
        // Extract encryption parameters.
        auto &context_data = *context_->context_data(encrypted.parms_id());
    ...
    void Evaluator::transform_to_ntt_inplace(Plaintext &plain, parms_id_type parms_id,
        const MemoryPoolHandle &pool)
    {
        // Verify parameters.
    ...
    void Evaluator::transform_from_ntt_inplace(Ciphertext &encrypted_ntt)
    {
        ...
        // Finally change the is_ntt_transformed flag
        encrypted_ntt.is_ntt_form_ = false;
    }
        ...
        const GaloisKeys &galois_keys, const MemoryPoolHandle &pool)
    {
        ...
        // Verify parameters
        if (!(galois_elt & 1) || (galois_elt >= 2 * static_cast<uint64_t>(coeff_count)))
        {
        ...
        uint64_t n = coeff_count;
        uint64_t m = n << 1;
        uint64_t subgroup_size = n >> 1;
        ...
        const GaloisKeys &galois_keys, const MemoryPoolHandle &pool)
    {
        ...
        bool sign = steps < 0;
        uint32_t pos_steps = static_cast<uint32_t>(abs(steps));
        uint32_t n = static_cast<uint32_t>(context_data.parms().poly_modulus_degree());
        uint32_t m_power_of_two = static_cast<uint32_t>(get_power_of_two(n)) + 1;
        ...
        pos_steps &= (uint32_t(1) << m_power_of_two) - 1;
        if (sign)
        {
            steps = static_cast<int>(n >> 1) - static_cast<int>(pos_steps);
        }
        else
        {
            steps = static_cast<int>(pos_steps);
        }

        // Construct Galois element for row rotation
        uint64_t gen = 3;
        uint64_t galois_elt = 1ULL;
        while(steps--) 
        {
            galois_elt *= gen;
        ...
        apply_galois_inplace(encrypted, galois_elt, galois_keys, pool);
    }
}
