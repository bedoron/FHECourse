...
namespace seal
{
    ...
    @par NTT form
    When using the BFV scheme (scheme_type::BFV), all plaintexts and ciphertexts should 
    ...
    */
    ...
    public:
        ...
        */
        Evaluator(const std::shared_ptr<SEALContext> &context);
        ...
        */
        ...
        down to 2. If the size of encrypted is K+1, the given relinearization keys need to 
        ...
        */
        inline void relinearize_inplace(Ciphertext &encrypted, const RelinKeys &relin_keys,
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            relinearize_internal(encrypted, relin_keys, 2, pool);
        }

        /**
        Relinearizes a ciphertext. This functions relinearizes encrypted, reducing its size
        down to 2, and stores the result in the destination parameter. If the size of encrypted
        is K+1, the given relinearization keys need to have size at least K-1. Dynamic memory
        allocations in the process are allocated from the memory pool pointed to by the given
        MemoryPoolHandle.
        ...
        */
        inline void mod_switch_to_next_inplace(Ciphertext &encrypted, 
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            mod_switch_to_next(encrypted, encrypted, pool);
        }
        ...
        */
        ...
        modulus down until the parameters reach the given parms_id.
        ...
        */
        ...
        @throws std::invalid_argument if encrypted is not valid for the encryption parameters
        @throws std::invalid_argument if encrypted is not in the default NTT form
guaranteed to be         @throws std::invalid_argument if encrypted is already at lowest level
        @throws std::invalid_argument if encrypted has size larger than 2
        @throws std::invalid_argument if pool is uninitialized
        */
        inline void rescale_to_next_inplace(Ciphertext &encrypted, 
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            rescale_to_next(encrypted, encrypted, pool);
        }
        ...
        */
        ...
        {
            destination = encrypted;
            rescale_to_inplace(destination, parms_id, pool);
        }
        ...
        */
        ...
        given relinearization keys are used.
        ...
        */
        ...
        {
            destination = encrypted;
            exponentiate_inplace(destination, exponent, relin_keys, pool);
        }
        ...
        */
        ...
        @param[out] destination The ciphertext to overwrite with the multiplication result
        @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
        @throws std::invalid_argument if the encrypted or plain is not valid for the
        encryption parameters
        @throws std::invalid_argument if encrypted and plain are in different NTT forms
        @throws std::invalid_argument if plain is zero
        @throws std::invalid_argument if, when using scheme_type::CKKS, the output scale
        ...
        */
        ...
        @throws std::invalid_argument if plain or parms_id is not valid for the encryption
        parameters
        @throws std::invalid_argument if pool is uninitialized
        */
        ...
        */
            ...
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            ...
            transform_to_ntt_inplace(destination_ntt, parms_id, pool);
        }
        ...
        */
        ...
        encryption parameters
        @throws std::invalid_argument if galois_keys do not correspond to the top level
        parameters in the current context
        @throws std::invalid_argument if encrypted is not in the default NTT form
        @throws std::invalid_argument if encrypted has size larger than 2
        ...
        */
        ...
        {
            destination = encrypted;
            apply_galois_inplace(destination, galois_elt, galois_keys, pool);
        }
        ...
        */
            ...
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            ...
            rotate_internal(encrypted, steps, galois_keys, pool);
        }
        ...
        */
        ...
        /**
        Rotates plaintext matrix columns cyclically. When batching is used with the BFV 
        scheme, this function rotates the encrypted plaintext matrix columns cyclically. 
        Since the size of the batched matrix is 2-by-(N/2), where N is the degree of the 
        ...
        */
            ...
            const MemoryPoolHandle &pool = MemoryManager::GetPool())
        {
            ...
            conjugate_internal(encrypted, galois_keys, pool);
        }

        /**
        Rotates plaintext matrix columns cyclically. When batching is used with the BFV
        scheme, this function rotates the encrypted plaintext matrix columns cyclically, 
        and writes the result to the destination parameter. Since the size of the batched 
        matrix is 2-by-(N/2), where N is the degree of the polynomial modulus, this means 
        simply swapping the two rows. Dynamic memory allocations in the process are 
        allocated from the memory pool pointed to by the given MemoryPoolHandle.
    ...
    private:
        ...
        void bfv_square(Ciphertext &encrypted, const MemoryPoolHandle &pool);

        void ckks_square(Ciphertext &encrypted, const MemoryPoolHandle &pool);

        void relinearize_internal(Ciphertext &encrypted, const RelinKeys &relin_keys,
            std::size_t destination_size, const MemoryPoolHandle &pool);
            ...
            const GaloisKeys &galois_keys, const MemoryPoolHandle &pool)
        {
            ...
            std::size_t poly_modulus_degree = context_data.parms().poly_modulus_degree();
            std::uint64_t m = static_cast<std::uint64_t>(poly_modulus_degree) << 1;
            ...
            const MemoryPoolHandle &pool)
        {
            ...
            std::size_t coeff_mod_count = coeff_modulus.size();
            std::size_t rns_poly_uint64_count = coeff_mod_count * coeff_count;
#ifdef SEAL_DEBUG
            if (value == nullptr)
            {
                throw std::invalid_argument("value cannot be null");
        ...
        void bfv_relinearize_one_step(std::uint64_t *encrypted, std::size_t encrypted_size,
            ...
            const RelinKeys &relin_keys, const MemoryPoolHandle &pool);
