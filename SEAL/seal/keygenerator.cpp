...
namespace seal
{
    KeyGenerator::KeyGenerator(const std::shared_ptr<SEALContext> &context) :
        context_(context)
    {
        // Verify parameters
        if (!context_)
    ...
    void KeyGenerator::generate_pk()
    {
        ...
        for (size_t i = 0; i < coeff_mod_count; i++)
        {
            ...
            ntt_negacyclic_harvey_lazy(
                public_key_1 + (i * coeff_count), small_ntt_tables[i]);
        }
        ...
        for (size_t k = 0; k < count; k++)
        {
            for (size_t l = 0; l < coeff_mod_count; l++)
            {
                ...
                for (size_t i = 0; i < decomposition_factors[l].size(); i++)
                {
                    ...
                    {
                        ntt_negacyclic_harvey_lazy(
                            eval_keys_second + (j * coeff_count), small_ntt_tables[j]);
    ...
    void KeyGenerator::set_poly_coeffs_zero_one_negone(
        ...
        uint64_t *poly, std::shared_ptr<UniformRandomGenerator> random) const
    {
        // Extract encryption parameters.
        auto &parms = context_data.parms();
        auto &coeff_modulus = parms.coeff_modulus();
    ...
    void KeyGenerator::populate_decomposition_factors(
        const SEALContext::ContextData &context_data, int decomposition_bit_count,
