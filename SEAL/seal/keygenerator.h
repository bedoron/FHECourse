...
namespace seal
{
    ...
    */
    ...
    public:
        ...
        */
        KeyGenerator(const std::shared_ptr<SEALContext> &context);
        ...
        */
        ...
        /**
        Generates and returns Galois keys. This function creates specific Galois keys 
        that can be used to apply specific Galois automorphisms on encrypted data. 
        The user needs to give as input a vector of Galois elements corresponding to 
        the keys that are to be created. Memory for the Galois keys is allocated from 
        the memory pool pointed to by the given MemoryPoolHandle.
        
        The Galois elements are odd integers in the interval [1, M-1], where M = 2*N, 
        and N = degree(poly_modulus). Used with batching, a Galois element 3^i % M 
        corresponds to a cyclic row rotation i steps to the left, and a Galois element 
        3^(N/2-i) % M corresponds to a cyclic row rotation i steps to the right. The 
        Galois element M-1 corresponds to a column rotation (row swap) in BFV, and 
        ...
        */
        ...
        /**
        Generates and returns Galois keys. This function creates logarithmically many 
        (in degree of the polynomial modulus) Galois keys that is sufficient to apply 
        any Galois automorphism (e.g. rotations) on encrypted data. Most users will 
        want to use this overload of the function. Memory for the Galois keys is 
    ...
    private:
        ...
        */
        ...
        /**
        Generates new public key matching to existin secret key.
        */
        void generate_pk();
        ...
        std::size_t secret_key_array_size_;
