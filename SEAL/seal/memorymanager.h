...
namespace seal
{
    ...
    */
    ...
    /**
    Control options for MemoryManager::GetPool function. These force the
    MemoryManager to override the current MMProf and instead
    return a MemoryPoolHandle pointing to a memory pool of the indicated
    type. 
    */
    ...
    /**
    The MMProf is a pure virtual class that every profile for the 
    MemoryManager should inherit from. The only functionality this class 
    implements is the get_pool(mm_prof_opt_t) function that returns 
    a MemoryPoolHandle pointing to a pool selected by internal logic 
    optionally using the input parameter of type mm_prof_opt_t.
    The returned MemoryPoolHandle must point to a valid memory pool.
    */
    class MMProf
    {
    public:
        ...
        */
        ...
        /**
        Returns a MemoryPoolHandle pointing to a pool selected by
        internal logic in a derived class and by the mm_prof_opt_t
        input parameter.
        */
    ...
    /**
    A memory manager profile that always returns a MemoryPoolHandle 
    pointing to the global memory pool. SEAL uses this memory manager 
    profile by default.
    */
    ...
    public:
        ...
        */
        virtual ~MMProfGlobal() override
        {
        }

        /**
        Returns a MemoryPoolHandle pointing to the global memory pool. 
        The mm_prof_opt_t input parameter has no effect.
        */
    ...
    /**
    A memory manager profile that always returns a MemoryPoolHandle
    pointing to the new thread-safe memory pool. This profile should 
    not be used except in special circumstances, as it does not result 
    in any reuse of allocated memory.
    */
    ...
    public:
        ...
        */
        virtual ~MMProfNew() override
        {
        }

        /**
        Returns a MemoryPoolHandle pointing to a new thread-safe 
        memory pool. The mm_prof_opt_t input parameter has no effect.
        */
        inline virtual MemoryPoolHandle 
            get_pool(mm_prof_opt_t) override
        {
    ...
    /**
    A memory manager profile that always returns a MemoryPoolHandle 
    pointing to specific memory pool.
    */
    class MMProfFixed : public MMProf
    {
    public:
        /**
        Creates a new MMProfFixed. The MemoryPoolHandle given as argument
        is returned by every call to get_pool(mm_prof_opt_t).

        @param[in] pool The MemoryPoolHandle pointing to a valid memory pool
        @throws std::invalid_argument if pool is uninitialized        
        */
        MMProfFixed(const MemoryPoolHandle &pool)
        {
            if (!pool)
            {
        ...
        */
        virtual ~MMProfFixed() override
        {
        }

        /**
        Returns a MemoryPoolHandle pointing to the stored memory 
        pool. The mm_prof_opt_t input parameter has no effect.
        */
        inline virtual MemoryPoolHandle
            get_pool(mm_prof_opt_t) override
        {
    ...
    /**
    A memory manager profile that always returns a MemoryPoolHandle
    pointing to the thread-local memory pool. This profile should be
    used with care, as any memory allocated by it will be released
    once the thread exits. In other words, the thread-local memory
    pool cannot be used to share memory across different threads.
    On the other hand, this profile can be useful when a very high
    number of threads doing simultaneous allocations would cause
    ...
    */
    ...
    public:
        ...
        */
        virtual ~MMProfThreadLocal() override
        {
        }

        /**
        Returns a MemoryPoolHandle pointing to the thread-local
        memory pool. The mm_prof_opt_t input parameter has no effect.
        */
        inline virtual MemoryPoolHandle 
            get_pool(mm_prof_opt_t) override
    ...
    /**
    The MemoryManager class can be used to create instances of
    MemoryPoolHandle based on a given "profile". A profile is
    implemented by inheriting from the MMProf class (pure virtual)
    and encapsulates internal logic for deciding which memory pool 
    to use.
    */
    class MemoryManager
    {
    ...
    public:
        ...
        /**
        Sets the current profile to a given one and returns a unique_ptr
        pointing to the previously set profile.
        ...
        */
        ...
        /**
        Returns a MemoryPoolHandle according to the currently set memory 
        manager profile and prof_opt. The following values for prof_opt
        have an effect independent of the current profile:
        ...
        @param[in] prof_opt A mm_prof_opt_t parameter used to provide
        additional instructions to the memory manager profile for internal
        logic.
        */
        static inline MemoryPoolHandle GetPool(
            mm_prof_opt_t prof_opt = mm_prof_opt::DEFAULT)
        {
            switch (prof_opt)
            {
    ...
    /**
    Class for a scoped switch of memory manager profile. This class acts
    as a scoped "guard" for changing the memory manager profile so that the
    programmer does not have to explicitly switch back afterwards and that
    other threads cannot change the MMProf. It can also help with exception 
    safety by guaranteeing that the profile is switched back to the original 
    if a function throws an exception after changing the profile for local 
    use.
    */
    class MMProfGuard
    {
    public:
        /**
        Creates a new MMProfGuard. If start_locked is true, this function
        will attempt to lock the MemoryManager for profile switch to mm_prof, 
        perform the switch, and keep the lock until unlocked or destroyed. If
        start_lock is false, mm_prof will be stored but the switch will not
        be performed and a lock will not be obtained until lock() is explicitly
        called.
        ...
        */
        ...
        /**
        Attempts to lock the MemoryManager for profile switch, perform the 
        switch to currently stored memory manager profile, store the previously
        held profile, and keep the lock until unlocked or destroyed. If the
        lock cannot be obtained on the first attempt, the function returns 
        ...
        */
        ...
        /**
        Locks the MemoryManager for profile switch, performs the switch to 
        currently stored memory manager profile, stores the previously held 
        profile, and keep the lock until unlocked or destroyed. The calling
        thread will block until the lock can be obtained.
        ...
        */
        ...
        /**
        Attempts to lock the MemoryManager for profile switch, perform the
        switch to the given memory manager profile, store the previously
        held profile, and keep the lock until unlocked or destroyed. If the
        lock cannot be obtained on the first attempt, the function returns
        false; otherwise returns true.
        ...
        */
        ...
        /**
        Locks the MemoryManager for profile switch, performs the switch to
        the given memory manager profile, stores the previously held profile, 
        and keep the lock until unlocked or destroyed. The calling thread 
        ...
        */
        ...
        /**
        Releases the memory manager profile switch lock for MemoryManager, 
        stores the current profile, and resets the profile to the one used 
        before locking.
        ...
        */
        ...
        /**
        Destroys the MMProfGuard. If the memory manager profile switch lock 
        is owned, releases the lock, and resets the profile to the one used 
        before locking.
        */
        ~MMProfGuard()
        {
        ...
        /**
        Returns whether the current MMProfGuard owns the memory manager
        profile switch lock.
        */
