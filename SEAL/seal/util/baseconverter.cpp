...
namespace seal
{
    namespace util
    {
            ...
            size_t coeff_count, const SmallModulus &small_plain_mod)
        {
            ...
            if (coeff_base.size() > SEAL_COEFF_MOD_COUNT_MAX)
            {
            ...
            */
            ...
            if (32 + small_plain_mod_.bit_count() + total_coeff_bit_count >= 
                61 * static_cast<int>(coeff_base_mod_count_) + 61)
            {
                aux_base_mod_count_++;
            }
        ...
        void BaseConverter::fastbconv(const uint64_t *input, uint64_t *destination, 
            ...
            */
            ...
            for (size_t i = 0; i < coeff_base_mod_count_; i++)
            {
                ...
                for (size_t k = 0; k < coeff_count_; k++, input++)
                {
                    temp_coeff_transition[i + (k * coeff_base_mod_count_)] = multiply_uint_uint_mod(
                        *input, inv_coeff_base_products_mod_coeff_elt, coeff_base_array_elt);
                }
            }
            ...
            for (size_t j = 0; j < bsk_base_mod_count_; j++)
            {
                ...
                for (size_t k = 0; k < coeff_count_; k++, destination++)
                {
                    ...
                    for (size_t i = 0; i < coeff_base_mod_count_; i++, temp_coeff_transition_ptr++,
        ...
        void BaseConverter::fastbconv_sk(const uint64_t *input, uint64_t *destination, 
            const MemoryPoolHandle &pool) const
        {
#ifdef SEAL_DEBUG
            if (input == nullptr)
            ...
            */
            ...
            for (size_t k = 0; k < bsk_base_mod_count_; k++)
            {
                    ...
                    input_m_tilde_ptr_copy++, input++)
                {
                    ...
                    *destination = multiply_uint_uint_mod(r_mtilde, inv_mtilde_mod_bsk_array_elt, 
                        bsk_base_array_elt);
                }
            }
        }

        void BaseConverter::fast_floor(const uint64_t *input, uint64_t *destination, 
            const MemoryPoolHandle &pool) const
        {
#ifdef SEAL_DEBUG
            if (input == nullptr)
        ...
        void BaseConverter::fastbconv_mtilde(const uint64_t *input, uint64_t *destination, 
            const MemoryPoolHandle &pool) const
        {
#ifdef SEAL_DEBUG
            if (input == nullptr)
            ...
            */
            ...
            for (size_t i = 0; i < coeff_base_mod_count_; i++)
            {
                ...
                for (size_t k = 0; k < coeff_count_; k++, input++)
                {
                    temp_coeff_transition[i + (k * coeff_base_mod_count_)] = 
                        multiply_uint_uint_mod(*input, mtilde_inv_coeff_base_products_mod_coeff_elt, 
            ...
            for (size_t j = 0; j < bsk_base_mod_count_; j++)
            {
                ...
                for (size_t k = 0; k < coeff_count_; k++, destination_ptr++)
                {
                    ...
                    for (size_t i = 0; i < coeff_base_mod_count_; i++, temp_ptr++,
            ...
            for (size_t k = 0; k < coeff_count_; k++, destination++)
            {
                ...
                for (size_t i = 0; i < coeff_base_mod_count_; i++,
                    temp_coeff_transition_ptr++, coeff_base_products_mod_mtilde_array_ptr++)
                {
                    // Lazy reduction
        ...
        void BaseConverter::fastbconv_plain_gamma(const uint64_t *input, uint64_t *destination, 
            const MemoryPoolHandle &pool) const
        {
#ifdef SEAL_DEBUG
            if (small_plain_mod_.is_zero())
            ...
            */
            ...
            for (size_t i = 0; i < coeff_base_mod_count_; i++)
            {
                ...
                {
                    temp_coeff_transition[i + (k * coeff_base_mod_count_)] = 
                        multiply_uint_uint_mod(*input, inv_coeff_base_products_mod_coeff_elt, 
                        coeff_base_array_elt);
                }
            }
