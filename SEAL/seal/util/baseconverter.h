...
namespace seal
{
    namespace util
    {
        ...
        {
        public:
            BaseConverter(const MemoryPoolHandle &pool) : pool_(pool)
            {
#ifdef SEAL_DEBUG
                if (!pool_)
                {
            ...
            */
            BaseConverter(const std::vector<SmallModulus> &coeff_base, std::size_t coeff_count, 
                const SmallModulus &small_plain_mod, const MemoryPoolHandle &pool);

            /**
            Generates the pre-computations for the given parameters.
            */
            void generate(const std::vector<SmallModulus> &coeff_base, std::size_t coeff_count,
            ...
            */
            ...
            inline auto is_generated() const
            {
                return generated_;
            }

            inline auto coeff_base_mod_count() const
            {
                return coeff_base_mod_count_;
            }

            inline auto aux_base_mod_count() const
            {
                return aux_base_mod_count_;
            }

            inline auto &get_plain_gamma_product() const
            {
                return plain_gamma_product_mod_coeff_array_;
            }

            inline auto &get_neg_inv_coeff() const
            {
                return neg_inv_coeff_products_all_mod_plain_gamma_array_;
            }

            inline auto &get_plain_gamma_array() const
            {
                return plain_gamma_array_;
            }

            inline const std::uint64_t *get_coeff_products_array() const
            {
            ...
            inline auto &get_bsk_small_ntt_tables() const
            {
                return bsk_small_ntt_tables_;
            }

            inline auto bsk_base_mod_count() const
            {
                return bsk_base_mod_count_;
            }

            inline auto &get_bsk_mod_array() const
            {
                return bsk_base_array_;
            }

            inline auto &get_msk() const
            {
                return m_sk_;
            }

            inline auto &get_m_tilde() const
            {
                return m_tilde_;
            }

            inline auto &get_mtilde_inv_coeff_products_mod_coeff() const
            {
                return mtilde_inv_coeff_base_products_mod_coeff_array_;
            }

            inline auto &get_inv_coeff_mod_mtilde() const
            {
                return inv_coeff_products_mod_mtilde_;
            }

            inline auto &get_inv_coeff_mod_coeff_array() const
            {
                return inv_coeff_base_products_mod_coeff_array_;
            }

            inline auto &get_inv_last_coeff_mod_array() const
            {
                return inv_last_coeff_mod_array_;
            }

            inline auto &get_coeff_base_products_mod_msk() const
            {
