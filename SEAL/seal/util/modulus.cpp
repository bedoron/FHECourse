    ...
    namespace util
    {
        namespace
        {
            bool is_inverse_small(const uint64_t *operand, int bit_count)
            {
                ...
                if (is_zero_uint(operand, static_cast<size_t>(divide_round_up(bit_count, bits_per_uint64))))
                {
                    throw invalid_argument("operand");
                }
#endif
                ...
                bit_count--;
                size_t uint64_count = static_cast<size_t>(divide_round_up(bit_count, bits_per_uint64));
                ...
                for (size_t i = 0; i < uint64_count; ++i)
                {
                    ...
                    if (sum != 0)
                    {
                        sig_bit_count = get_significant_bit_count(sum) + 
                            static_cast<int>(i * bits_per_uint64);
                    }
                }
        ...
        Modulus::Modulus(const std::uint64_t *modulus, size_t uint64_count) : 
            modulus_(modulus), uint64_count_(uint64_count)
        {
            ...
            if (is_inverse_small(modulus, significant_bit_count_))
            {
                ...
                inverse_modulus_ = decltype(inverse_modulus_)::Owning(new uint64_t[uint64_count]);
        ...
        Modulus::Modulus(const std::uint64_t *modulus, size_t uint64_count, MemoryPool &pool) : 
            modulus_(modulus), uint64_count_(uint64_count)
        {
#ifdef SEAL_DEBUG
            if (modulus == nullptr)
        ...
        Modulus &Modulus::operator=(const Modulus &assign)
        {
            ...
            {
                inverse_modulus_ = decltype(inverse_modulus_)::Owning(new uint64_t[uint64_count_]);
        ...
        Modulus::Modulus(Modulus &&source) noexcept : 
            modulus_(source.modulus_), uint64_count_(source.uint64_count_), 
