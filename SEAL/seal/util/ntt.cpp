    ...
    namespace util
    {
        namespace
        {
            namespace tools
            {
                ...
                inline void left_shift_one_bit_uint(const uint64_t *operand, size_t uint64_count, uint64_t *result)
                {
                ...
                inline void right_shift_one_bit_uint(const uint64_t *operand, size_t uint64_count, uint64_t *result)
                {
                ...
                void multiply_truncate_uint_uint_add(const uint64_t *operand1, const uint64_t *operand2, 
                    size_t uint64_count, uint64_t *result)
                {
                    ...
                    for (size_t operand1_index = 0; operand1_index < uint64_count; operand1_index++)
                    {
                ...
                void multiply_truncate_uint_uint_sub(const uint64_t *operand1, const uint64_t *operand2, 
                    size_t uint64_count, uint64_t *result)
                {
                ...
                void multiply_uint_uint(const uint64_t *operand1, const uint64_t *operand2, 
                    size_t operand_uint64_count, uint64_t *result)
                {
                    size_t result_uint64_count = 2 * operand_uint64_count;
                    ...
                    for (size_t operand1_index = 0; operand1_index < operand_uint64_count; operand1_index++)
                    {
                        ...
                        uint64_t *inner_result = result++;
                        uint64_t carry = 0;
                        for (size_t operand2_index = 0; operand2_index < operand_uint64_count; operand2_index++)
                        {
                            // Perform 64-bit multiplication of operand1 and operand2
                            unsigned long long temp_result[2];
                            unsigned long long temp;
        ...
        {
            int word_bit_count = static_cast<int>(modulus.uint64_count()) * bits_per_uint64;
            ...
            auto inverse_root(allocate_uint(coeff_uint64_count_, pool_));
            ...
            auto degree_uint(allocate_zero_uint(coeff_uint64_count_, pool_));
        ...
        void NTTTables::ntt_powers_of_primitive_root(uint64_t *root, uint64_t *destination)
        {
            ...
            {
                uint64_t *next_destination = destination_start + 
                    coeff_uint64_count_ * reverse_bits(i, coeff_count_power_);
        ...
        {
            size_t wide_uint64_count = 2 * coeff_uint64_count_;

            auto wide_coeff(allocate_uint(wide_uint64_count, pool_));
            auto wide_modulus(allocate_uint(wide_uint64_count, pool_));
            set_uint_uint(modulus_.get(), coeff_uint64_count_, wide_uint64_count, wide_modulus.get());

            auto wide_quotient(allocate_uint(wide_uint64_count, pool_));
            auto wide_remainder(allocate_uint(wide_uint64_count, pool_));

            for (size_t i = 0; i < coeff_count_; i++)
            {
                set_uint_uint(input, coeff_uint64_count_, wide_uint64_count, wide_coeff.get());
                left_shift_uint(wide_coeff.get(), static_cast<int>(coeff_uint64_count_) * bits_per_uint64, 
                    wide_uint64_count, wide_coeff.get());
                divide_uint_uint(wide_coeff.get(), wide_modulus.get(), wide_uint64_count, 
                    wide_quotient.get(), wide_remainder.get(), pool_);
        ...
        */
        ...
        {
            size_t coeff_uint64_count = tables.coeff_uint64_count(); 

            const uint64_t *modulusptr = tables.modulus().get(); 
            auto alloc_anchor(allocate_uint(5 * coeff_uint64_count, pool));
        ...
        void inverse_ntt_negacyclic_harvey(uint64_t *operand, const NTTTables &tables, MemoryPool &pool)
        {
            ...
            auto alloc_anchor(allocate_uint(4 * coeff_uint64_count, pool));
            uint64_t *two_times_modulus = alloc_anchor.get();
            uint64_t *T = two_times_modulus + coeff_uint64_count;
            uint64_t *prod = T + coeff_uint64_count;
