...
namespace seal
{
    namespace util
    {
        namespace
        {
            const size_t use_base_case = 8;

            namespace tools
            {
                inline void small_right_shift_sign_extend_uint(const uint64_t *operand, 
                ...
                inline void fold(const uint64_t *operand, size_t coeff_uint64_count, 
                    size_t m, size_t r, uint64_t *result)
                {
                    for (size_t indexM = 0; indexM < m; indexM++)
                    {
                        ...
                        for (size_t indexR = 1; indexR < r; indexR++)
                        {
                ...
                inline void rearrange(const uint64_t *operand, size_t coeff_uint64_count, 
                    size_t m, size_t r, uint64_t *result)
                {
                    for (size_t indexM = 0; indexM < m; indexM++)
                    {
                        ...
                        for (size_t indexR = 0; indexR < r; indexR++)
                        {
                    ...
                    set_poly_poly(result, m * r, coeff_uint64_count, 
                        get_poly_coeff(result, m * r, coeff_uint64_count));
                }

                // Perform inplace one round less DFT. 
                void symbolic_dft_oneroundless(uint64_t *operand, int coeff_count_power, 
                    size_t coeff_uint64_count, size_t m, size_t r, uint64_t *alloc_ptr)
                {
                    ...
                    for (int outer_index = outer_start; outer_index >= 0; outer_index--)
                    {
                        ...
                        size_t middle_end = size_t(1) << outer_remaining;		// middle_end = "m".
                        size_t inner_end = size_t(1) << outer_index;			// inner_end = t.
                                                                    // In the Cooley-Tukey, we always have the relation 2mt = n. 
                                                                    // So in this code, we have n =  2 * (coeff_count_power / 2) = 2M.
                        for (size_t middle_index = 0; middle_index < middle_end; middle_index++) // middle_index = i. 
                        {
                            size_t sr = reverse_bits(middle_index, outer_remaining) << outer_index; //scrambling.
                            size_t s = middle_index << (outer_index + 1);	// s = middle_index*2t = j1
                            size_t k = (r / m) * sr;						// The root of unity. 
                            for (size_t inner_index = 0; inner_index < inner_end; inner_index++)
                            {
                                // inner_end =  t. As we know t is decreasing. 
                                // inner index is like j?
                                // s -- okay.
                                size_t i = s + inner_index; // j 
                                size_t l = i + inner_end; // j + t.
                                ...
                                for (size_t a = 0; a < r; a++)
                                {
                                    ...
                                    unsigned char borrow = sub_uint64(*x_coeff_ptr, *temp_poly_coeff_ptr, 0, &temp);
                                    *x_coeff_ptr2++ = temp;
                                    unsigned char carry = add_uint64(*x_coeff_ptr, *temp_poly_coeff_ptr++, 0, &temp);
                                    *x_coeff_ptr++ = temp;
                                    for (size_t add_sub_index = 1; add_sub_index < coeff_uint64_count; add_sub_index++)
                                    {
                                        borrow = sub_uint64(*x_coeff_ptr, *temp_poly_coeff_ptr, borrow, &temp);
                                        *x_coeff_ptr2++ = temp;
                                        carry = add_uint64(*x_coeff_ptr, *temp_poly_coeff_ptr++, carry, &temp);
                                        *x_coeff_ptr++ = temp;
                                    }
                                }
                            }
                ...
                void symbolic_inverse_dft(uint64_t *operand, int coeff_count_power, 
                    size_t coeff_uint64_count, size_t m, size_t r, uint64_t *alloc_ptr)
                {
                    ...
                    {
                        int outer_remaining = coeff_count_power / 2 - outer_index;
                        size_t middle_end = size_t(1) << outer_remaining;
                        size_t inner_end = size_t(1) << outer_index;
                        for (size_t middle_index = 0; middle_index < middle_end; middle_index++)
                        {
                            size_t sr = reverse_bits(middle_index, outer_remaining) << outer_index;
                            size_t s = middle_index << (outer_index + 1);
                            size_t k = (r / m) * sr;
                            for (size_t inner_index = 0; inner_index < inner_end; inner_index++)
                            {
                                size_t i = s + inner_index;
                                size_t l = i + inner_end;
                                ...
                                for (size_t a = 0; a < r; a++)
                                {
                                    // Add and subtract
                                    unsigned long long temp;
                                    unsigned char borrow = sub_uint64(*coeff1_ptr, *coeff2_ptr, 0, &temp);
                                    *dest_coeff_ptr++ = temp;
                                    unsigned char carry = add_uint64(*coeff1_ptr, *coeff2_ptr++, 0, &temp);
                                    *coeff1_ptr++ = temp;
                                    for (size_t add_sub_index = 1; add_sub_index < coeff_uint64_count; add_sub_index++)
                                    {
                                        borrow = sub_uint64(*coeff1_ptr, *coeff2_ptr, borrow, &temp);
                                        *dest_coeff_ptr++ = temp;
                                        carry = add_uint64(*coeff1_ptr, *coeff2_ptr++, carry, &temp);
                                        *coeff1_ptr = temp;
                                        coeff1_ptr++;
                                    }
                                }
                    ...
                    for (size_t i = 0; i < 2 * (size_t(1) << coeff_count_power); i++)
                    {
                ...
                void multiply_uint_uint_add(const uint64_t *operand1, const uint64_t *operand2, 
                    size_t operand_uint64_count, size_t result_uint64_count, uint64_t *result)
                {
                    // Handle fast cases.
                    ...
                    for (size_t operand1_index = 0; operand1_index < operand_uint64_count; operand1_index++)
                    {
                        const uint64_t *inner_operand2 = operand2;
                        uint64_t *inner_result = result++;
                        unsigned long long carry = 0;
                        size_t operand2_index = 0;
                        size_t operand2_index_max = min(operand_uint64_count, result_uint64_count - operand1_index);
                        for (; operand2_index < operand2_index_max; operand2_index++)
                        {
                            ...
                            carry += add_uint64(*inner_result, temp_result[0], 0, temp_result);
                            *inner_result++ = temp_result[0];
                        }
                        ...
                        {
                            unsigned long long temp;
                            unsigned char c_carry = add_uint64(*inner_result, carry, 0, &temp);
                            *inner_result++ = temp;
                            for (size_t i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result, 0ULL, c_carry, &temp);
                                *inner_result++ = temp;
                            }
                        }
                ...
                void triple_multiply_uint_uint_add(
                    const uint64_t *operand11, const uint64_t *operand12, const uint64_t *operand22, 
                    const uint64_t *operand31, size_t operand_uint64_count, size_t result_uint64_count,
                    uint64_t *result1, uint64_t *result2, uint64_t *result3)
                {
                    ...
                    for (size_t operand1_index = 0; operand1_index < operand_uint64_count; operand1_index++)
                    {
                        ...
                        {
                            unsigned long long temp;
                            unsigned char c_carry = add_uint64(*inner_result1, carry1, 0, &temp);
                            *inner_result1++ = temp;
                            for (size_t i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result1, 0ULL, c_carry, &temp);
                                *inner_result1++ = temp;
                            }

                            c_carry = add_uint64(*inner_result2, carry2, 0, &temp);
                            *inner_result2++ = temp;
                            for (size_t i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result2, 0ULL, c_carry, &temp);
                                *inner_result2++ = temp;
                            }

                            c_carry = add_uint64(*inner_result3, carry3, 0, &temp);
                            *inner_result3++ = temp;
                            for (size_t i = operand1_index + operand2_index + 1; i < result_uint64_count; i++)
                            {
                                c_carry = add_uint64(*inner_result3, 0ULL, c_carry, &temp);
                                *inner_result3++ = temp;
                            }
                        }
                ...
                inline void set_uint_uint_negate_if(const uint64_t *operand, size_t uint64_count, 
                    unsigned char negate, uint64_t *destination)
                {
                    // negate is supposed to be either 0 or 1
                    uint64_t negate_mask = ~(static_cast<uint64_t>(negate) - 1);
                    unsigned long long temp;
                    for (size_t k = 0; k < uint64_count; k++)
                    {
                        negate = add_uint64(*operand++ ^ negate_mask, 0ULL, negate, &temp);
                        *destination++ = temp;
                    }
                }
            }
            ...
            namespace nussbaumer_multiply
            {
                ...
                inline void do_nussbaumer_base_signcheck(uint64_t *operand1, uint64_t *operand2, 
                    int coeff_count_power, size_t sum_uint64_count, size_t product_uint64_count, 
                    uint64_t *result, uint64_t *alloc_ptr)
                {
                    if (coeff_count_power == 2)
                    {
                        ...
                        {
                            op_coeff_signs[i] = static_cast<unsigned char>(
                                *(operand_coeff_ptr + sum_uint64_count - 1) >> (bits_per_uint64 - 1));
                        ...
                        for (size_t i = 0; i < coeff_count; i++)
                        {
                            op_coeff_signs[coeff_count + i] = static_cast<unsigned char>(
                                *(operand_coeff_ptr + sum_uint64_count - 1) >> (bits_per_uint64 - 1));
                        ...
                        if(coeff_count * product_uint64_count > numeric_limits<unsigned char>::max())
                        {
                            throw invalid_argument("coefficients too large");
                        }
                        unsigned char positive_offset = static_cast<unsigned char>(coeff_count * product_uint64_count);
                        ...
                        for (size_t i = 0; i < coeff_count; i++)
                        {
                            ...
                            unsigned long long temp;
                            for (size_t j = 0; j < product_uint64_count; j++)
                            {
                                borrow = sub_uint64(*(alloc_ptr + positive_offset), *alloc_ptr, borrow, &temp);
                                *result++ = temp;
                                alloc_ptr++;
                            }
                        }
                ...
                void do_nussbaumer(uint64_t *operand1, uint64_t *operand2, int coeff_count_power, 
                    size_t sum_uint64_count, size_t product_uint64_count, uint64_t *result, MemoryPool &pool)
                {
                    // Handle base case differently.
                    size_t coeff_count = size_t(1) << coeff_count_power;
                    if (coeff_count <= use_base_case)
                    {
                        auto fft_base_alloc(allocate_uint(2 * coeff_count * product_uint64_count, pool));
                    ...
                    size_t m = size_t(1) << (coeff_count_power / 2);
                    int rec_coeff_count_power = (coeff_count_power + 1) / 2;
                    size_t r = size_t(1) << rec_coeff_count_power;

                    // To reduce the number of allocations, we allocate one big block of memory at once.
                    auto big_alloc_anchor(allocate_uint(2 * coeff_count * sum_uint64_count + 2 * coeff_count * sum_uint64_count + 2 * coeff_count * product_uint64_count
            ...
            {
                void do_nussbaumer_base_signcheck(uint64_t *operand1, uint64_t *operand2, 
                    int coeff_count_power, size_t sum_uint64_count, size_t product_uint64_count, 
                    uint64_t *result_11, uint64_t *result_22, uint64_t *result_12, uint64_t *alloc_ptr)
                {
                    if (coeff_count_power == 2)
                    {
                        const size_t coeff_count = 4;
                        ...
                        for (size_t i = 0; i < coeff_count; i++)
                        {
                            ...
                            for (size_t j = 0; j < product_uint64_count; j++)
                            {
                                borrow11 = sub_uint64(*(negative_result_11 + positive_offset), *negative_result_11, borrow11, &temp);
                                *result_11++ = temp;
                                negative_result_11++;
                                borrow12 = sub_uint64(*(negative_result_12 + positive_offset), *negative_result_12, borrow12, &temp);
                                *result_12++ = temp;
                                negative_result_12++;
                                borrow22 = sub_uint64(*(negative_result_22 + positive_offset), *negative_result_22, borrow22, &temp);
                                *result_22++ = temp;
                                negative_result_22++;
                            }
                        }
                ...
                void do_nussbaumer(uint64_t *operand1, uint64_t *operand2, int coeff_count_power, 
                    size_t sum_uint64_count, size_t product_uint64_count, uint64_t *result_op1_op1, 
                    uint64_t *result_op2_op2, uint64_t *result_op1_op2, MemoryPool &pool)
                {
                    // Handle base case differently.
                    size_t coeff_count = size_t(1) << coeff_count_power;
                    if (coeff_count <= use_base_case)
                    {
                        auto fft_base_alloc(allocate_uint(6 * coeff_count * product_uint64_count, pool));
                    ...
                    // To reduce the number of allocations, we allocate one big block of memory at once.
                    auto big_alloc_anchor(allocate_uint(2 * coeff_count * sum_uint64_count + 2 * coeff_count * sum_uint64_count + 
        ...
        void nussbaumer_multiply_poly_poly(const uint64_t *operand1, const uint64_t *operand2, 
            int coeff_count_power, size_t coeff_uint64_count, size_t sum_uint64_count, 
            size_t product_uint64_count, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (operand1 == nullptr)
            {
            ...
            auto operandsresized(allocate_uint(2 * coeff_count * sum_uint64_count, pool));
            ...
            nussbaumer_multiply::do_nussbaumer(operand1resized, operand2resized, 
                coeff_count_power, sum_uint64_count, product_uint64_count, result, pool);
        }
        
        void nussbaumer_cross_multiply_poly_poly(const uint64_t *operand1, const uint64_t *operand2, 
            int coeff_count_power, size_t coeff_uint64_count, size_t sum_uint64_count, 
            size_t product_uint64_count, uint64_t *result_op1_op1, uint64_t *result_op2_op2, 
            uint64_t *result_op1_op2, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (operand1 == nullptr)
            {
            ...
            nussbaumer_cross_multiply::do_nussbaumer(operand1resized, operand2resized, 
                coeff_count_power, sum_uint64_count, product_uint64_count, result_op1_op1, 
                result_op2_op2, result_op1_op2, pool);
        }
    }
}
