...
namespace seal
{
    namespace util
    {
        void multiply_poly_scalar_coeffmod(const uint64_t *poly, size_t coeff_count, 
            const uint64_t *scalar, const Modulus &modulus, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
...
#endif
            size_t coeff_uint64_count = modulus.uint64_count();
            size_t intermediate_uint64_count = coeff_uint64_count * 2;
            auto intermediate(allocate_uint(intermediate_uint64_count, pool));
        ...
        void multiply_poly_poly_coeffmod(const uint64_t *operand1, size_t operand1_coeff_count, 
            size_t operand1_coeff_uint64_count, const uint64_t *operand2, 
            size_t operand2_coeff_count, size_t operand2_coeff_uint64_count,
            const Modulus &modulus, size_t result_coeff_count, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
...
#endif
            size_t intermediate_uint64_count = operand1_coeff_uint64_count + operand2_coeff_uint64_count;
            auto intermediate(allocate_uint(intermediate_uint64_count, pool));

            // Clear product.
            size_t result_coeff_uint64_count = modulus.uint64_count();
            ...
            for (size_t operand1_index = 0; operand1_index < operand1_coeff_count; operand1_index++)
            {
                ...
                for (size_t operand2_index = 0; operand2_index < operand2_coeff_count; operand2_index++)
                {
                    ...
                    multiply_uint_uint(operand1_coeff, operand1_coeff_uint64_count, operand2_coeff, 
                        operand2_coeff_uint64_count, intermediate_uint64_count, intermediate.get());
                    ...
                    add_uint_uint_mod(result_coeff, intermediate.get(), modulusptr, 
        ...
        void divide_poly_poly_coeffmod_inplace(uint64_t *numerator, 
            const uint64_t *denominator, size_t coeff_count, const Modulus &modulus, 
            uint64_t *quotient, MemoryPool &pool)
        {
            ...
            size_t numerator_coeffs = get_significant_coeff_count_poly(
                numerator, coeff_count, coeff_uint64_count);
            size_t denominator_coeffs = get_significant_coeff_count_poly(
                denominator, coeff_count, coeff_uint64_count);
            ...
            size_t intermediate_uint64_count = coeff_uint64_count * 2;
            auto alloc_anchor(allocate_uint(
                coeff_uint64_count + intermediate_uint64_count + intermediate_uint64_count, pool));
            ...
            const uint64_t *modulusptr = modulus.get();
            const uint64_t *leading_denominator_coeff = get_poly_coeff(
                denominator, denominator_coeffs - 1, coeff_uint64_count);
            if (!try_invert_uint_mod(leading_denominator_coeff, modulusptr, 
                coeff_uint64_count, monic_denominator_scalar, pool))
            {
            ...
            while (numerator_coeffs >= denominator_coeffs)
            {
                ...
                const uint64_t *leading_numerator_coeff = get_poly_coeff(
                    numerator, numerator_coeffs - 1, coeff_uint64_count);
                ...
                if (!is_zero_uint(leading_numerator_coeff, coeff_uint64_count))
                {
                    ...
                    // coefficient of denominator (which when subtracted will zero out 
                    // the topmost denominator coefficient).
                    uint64_t *quotient_coeff = get_poly_coeff(
                        quotient, denominator_shift, coeff_uint64_count);
                    multiply_uint_uint_mod_inplace(monic_denominator_scalar, 
                        leading_numerator_coeff, modulus, temp_quotient, pool);
                        ...
                        denominator_coeff_index < denominator_coeffs; denominator_coeff_index++)
                    {
                        ...
                        const uint64_t *denominator_coeff = get_poly_coeff(
                            denominator, denominator_coeff_index, coeff_uint64_count);
                        multiply_uint_uint_mod_inplace(temp_quotient, 
                            denominator_coeff, modulus, subtrahend, pool);

                        // Subtract numerator with resulting product, appropriately 
                        // shifted by denominator shift.
                        uint64_t *numerator_coeff = get_poly_coeff(
                            numerator, denominator_coeff_index + denominator_shift, coeff_uint64_count);
                        sub_uint_uint_mod(numerator_coeff, subtrahend, modulusptr, 
                            coeff_uint64_count, numerator_coeff);
                    }
                }
        ...
        void modulo_poly_inplace(uint64_t *value, size_t value_coeff_count, 
            const PolyModulus &poly_modulus, const Modulus &modulus, MemoryPool &pool)
        {
            ...
            for (size_t i = 0; i < poly_modulus.coeff_count(); i++)
            {
                if (is_greater_than_or_equal_uint_uint(
                    poly_modulus.get() + (i * poly_modulus.coeff_uint64_count()), 
                    poly_modulus.coeff_uint64_count(), modulus.get(), modulus.uint64_count()))
                {
            ...
            size_t coeff_uint64_count = modulus.uint64_count();
            size_t value_coeffs = get_significant_coeff_count_poly(
                value, value_coeff_count, coeff_uint64_count);
            size_t poly_modulus_coeff_count = poly_modulus.coeff_count();
            ...
            if (poly_modulus.is_one_zero_one())
            {
                ...
                while (value_coeffs >= poly_modulus_coeff_count)
                {
                    ...
                    uint64_t *leading_value_coeff = get_poly_coeff(
                        value, value_coeffs - 1, coeff_uint64_count);
                    ...
                    {
                        // Determine shift necesarry to bring significant coefficients in alignment.
                        size_t poly_modulus_shift = value_coeffs - poly_modulus_coeff_count;

                        // Subtract top coefficient from bottom-shifted coefficient.
                        uint64_t *value_coeff = get_poly_coeff(
                            value, poly_modulus_shift, coeff_uint64_count);
                        sub_uint_uint_mod(value_coeff, leading_value_coeff, coeff_modulus, 
                            coeff_uint64_count, value_coeff);
            ...
            auto polymodptr(duplicate_poly_if_needed(poly_modulus.get(), 
                poly_modulus.coeff_count(), poly_modulus.coeff_uint64_count(),
                poly_modulus.coeff_count(), coeff_uint64_count, false, pool));

            size_t intermediate_uint64_count = coeff_uint64_count * 2;
            auto alloc_anchor(allocate_uint(coeff_uint64_count + 2 * intermediate_uint64_count, pool));
            ...
            const uint64_t *leading_poly_modulus_coeff = get_poly_coeff(
                polymodptr.get(), poly_modulus_coeff_count - 1, coeff_uint64_count);
            if (!try_invert_uint_mod(leading_poly_modulus_coeff, coeff_modulus, 
                coeff_uint64_count, monic_poly_modulus_scalar, pool))
            {
            ...
            {
                // Determine leading value coefficient.
                const uint64_t *leading_value_coeff = get_poly_coeff(
                    value, value_coeffs - 1, coeff_uint64_count);
                ...
                if (!is_zero_uint(leading_value_coeff, coeff_uint64_count))
                {
                    ...
                    // Determine quotient's coefficient, which is scalar that makes poly_modulus's 
                    // leading coefficient one multiplied by leading coefficient of poly_modulus 
                    // (which when subtracted will zero out the topmost poly_modulus coefficient).
                    multiply_uint_uint_mod_inplace(monic_poly_modulus_scalar, 
                        leading_value_coeff, modulus, temp_quotient, pool);

                    // Subtract value and quotient*poly_modulus (shifted by poly_modulus_shift).
                    for (size_t poly_modulus_coeff_index = 0; 
                        poly_modulus_coeff_index < poly_modulus_coeff_count; 
                        poly_modulus_coeff_index++)
                    {
                        // Multiply poly_modulus's coefficient by quotient.
                        const uint64_t *poly_modulus_coeff = get_poly_coeff(
                            polymodptr.get(), poly_modulus_coeff_index, coeff_uint64_count);
                        multiply_uint_uint_mod_inplace(temp_quotient, poly_modulus_coeff, 
                            modulus, subtrahend, pool);

                        // Subtract value with resulting product, appropriately shifted by poly_modulus shift.
                        uint64_t *value_coeff = get_poly_coeff(
                            value, poly_modulus_coeff_index + poly_modulus_shift, coeff_uint64_count);
                        sub_uint_uint_mod(value_coeff, subtrahend, coeff_modulus, 
                            coeff_uint64_count, value_coeff);
                    }
                }
        ...
        void nonfft_multiply_poly_poly_polymod_coeffmod(const uint64_t *operand1, 
            const uint64_t *operand2, const PolyModulus &poly_modulus, 
            const Modulus &modulus, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (operand1 == nullptr)
            {
            ...
            if (get_significant_coeff_count_poly(
                operand1, poly_modulus.coeff_count(), poly_modulus.coeff_uint64_count()) >= 
                poly_modulus.coeff_count())
            {
            ...
            for (size_t i = 0; i < poly_modulus.coeff_count(); i++)
            {
                if (is_greater_than_or_equal_uint_uint(poly_modulus.get() + 
                    (i * poly_modulus.coeff_uint64_count()), poly_modulus.coeff_uint64_count(),
            ...
            size_t coeff_count = poly_modulus.coeff_count();
            size_t coeff_uint64_count = poly_modulus.coeff_uint64_count();
            size_t intermediate_coeff_count = coeff_count * 2 - 1;
            auto intermediate(allocate_poly(intermediate_coeff_count, coeff_uint64_count, pool));
            multiply_poly_poly_coeffmod(operand1, operand2, coeff_count, modulus, 
                intermediate.get(), pool);

            // Perform modulo operation.
            modulo_poly_inplace(intermediate.get(), intermediate_coeff_count, 
                poly_modulus, modulus, pool);

            // Copy to result.
            set_poly_poly(intermediate.get(), coeff_count, coeff_uint64_count, result);
        }

        void nonfft_multiply_poly_poly_polymod_coeffmod_inplace(const uint64_t *operand1, 
            const uint64_t *operand2, const PolyModulus &poly_modulus, const Modulus &modulus, 
            uint64_t *result, MemoryPool &pool)
        {
            ...
            // Calculate normal product.
            size_t coeff_count = poly_modulus.coeff_count();
            size_t result_coeff_count = coeff_count * 2 - 1;
        ...
        bool try_invert_poly_coeffmod(const uint64_t *operand, const uint64_t *poly_modulus, 
            size_t coeff_count, const Modulus &modulus, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (operand == nullptr)
            {
            ...
            if (get_significant_coeff_count_poly(operand, coeff_count, 
                modulus.uint64_count()) >= get_significant_coeff_count_poly(
                    poly_modulus, coeff_count, modulus.uint64_count()))
            {
                throw out_of_range("operand");
            }
            for (size_t i = 0; i < coeff_count; i++)
            {
                if (is_greater_than_or_equal_uint_uint(poly_modulus + (i * modulus.uint64_count()), 
                    modulus.uint64_count(), modulus.get(), modulus.uint64_count()))
                {
                    throw invalid_argument("poly_modulus coefficients are not reduced modulo modulus");
                }
            }
            ...
            auto numerator_anchor(allocate_poly(coeff_count, coeff_uint64_count, pool));
            uint64_t *numerator = numerator_anchor.get();
            set_poly_poly(poly_modulus, coeff_count, coeff_uint64_count, numerator);
            auto denominator_anchor(allocate_poly(coeff_count, coeff_uint64_count, pool));
            ...
            auto quotient(allocate_poly(coeff_count, coeff_uint64_count, pool));

            // Create scalar to store value that makes denominator monic.
            auto monic_denominator_scalar(allocate_uint(coeff_uint64_count, pool));

            // Create temporary scalars used during calculation of quotient.
            // Both are purposely twice as wide to store intermediate product prior to modulo operation.
            size_t intermediate_uint64_count = coeff_uint64_count * 2;
            auto temp_quotient(allocate_uint(intermediate_uint64_count, pool));
            auto subtrahend(allocate_uint(intermediate_uint64_count, pool));

            // Create three polynomials to store inverse.
            // Initialize invert_prior to 0 and invert_curr to 1.
            auto invert_prior_anchor(allocate_poly(coeff_count, coeff_uint64_count, pool));
            uint64_t *invert_prior = invert_prior_anchor.get();
            set_zero_poly(coeff_count, coeff_uint64_count, invert_prior);
            auto invert_curr_anchor(allocate_poly(coeff_count, coeff_uint64_count, pool));
            ...
            auto invert_next_anchor(allocate_poly(coeff_count, coeff_uint64_count, pool));
            ...
            while (true)
            {
                ...
                const uint64_t *leading_denominator_coeff = get_poly_coeff(
                    denominator, denominator_coeffs - 1, coeff_uint64_count);
                if (!try_invert_uint_mod(leading_denominator_coeff, modulusptr, 
                    coeff_uint64_count, monic_denominator_scalar.get(), pool))
                {
                    throw invalid_argument("coeff_modulus is not coprime with leading denominator coefficient");
                }
                ...
                while (numerator_coeffs >= denominator_coeffs)
                {
                    ...
                    if (!is_zero_uint(leading_numerator_coeff, coeff_uint64_count))
                    {
                        ...
                        // Determine quotient's coefficient, which is scalar that makes denominator's 
                        // leading coefficient one multiplied by leading coefficient of denominator 
                        // (which when subtracted will zero out the topmost denominator coefficient).
                        uint64_t *quotient_coeff = get_poly_coeff(
                            quotient.get(), denominator_shift, coeff_uint64_count);
                        multiply_uint_uint_mod_inplace(monic_denominator_scalar.get(), 
                            leading_numerator_coeff, modulus, temp_quotient.get(), pool);
                            ...
                            denominator_coeff_index++)
                        {
                            ...
                            const uint64_t *denominator_coeff = get_poly_coeff(
                                denominator, denominator_coeff_index, coeff_uint64_count);
                            multiply_uint_uint_mod_inplace(temp_quotient.get(), 
                                denominator_coeff, modulus, subtrahend.get(), pool);

                            // Subtract numerator with resulting product, appropriately shifted 
                            // by denominator shift.
                            uint64_t *numerator_coeff = get_poly_coeff(
                                numerator, denominator_coeff_index + denominator_shift, coeff_uint64_count);
                            sub_uint_uint_mod(numerator_coeff, subtrahend.get(), modulusptr, 
                                coeff_uint64_count, numerator_coeff);
                        }
                    }
                ...
                multiply_truncate_poly_poly_coeffmod(quotient.get(), invert_curr, 
                    coeff_count, modulus, invert_next, pool);
                sub_poly_poly_coeffmod(invert_prior, invert_next, coeff_count, modulusptr, 
                    coeff_uint64_count, invert_next);
            ...
            // Determine scalar necessary to make denominator monic.
            const uint64_t *leading_denominator_coeff = get_poly_coeff(
                denominator, 0, coeff_uint64_count);
            if (!try_invert_uint_mod(leading_denominator_coeff, modulusptr, 
                coeff_uint64_count, monic_denominator_scalar.get(), pool))
            {
                throw invalid_argument("coeff_modulus is not coprime with leading denominator coefficient");
            ...
            multiply_poly_scalar_coeffmod(invert_curr, coeff_count, 
                monic_denominator_scalar.get(), modulus, result, pool);
            return true;
        }

        void poly_infty_norm_coeffmod(const uint64_t *poly, size_t poly_coeff_count, 
            size_t poly_coeff_uint64_count, const Modulus &modulus, uint64_t *result, 
            MemoryPool &pool)
        {
            // Construct negative threshold (first negative modulus value) to compute 
            // absolute values of coeffs.
            size_t modulus_uint64_count = modulus.uint64_count();
            auto modulus_neg_threshold(allocate_uint(modulus_uint64_count, pool));
            ...
            auto coeff_moded(allocate_uint(modulus_uint64_count, pool));
            uint64_t *coeffmodedptr = coeff_moded.get();
            for (size_t coeff_index = 0; coeff_index < poly_coeff_count; coeff_index++)
            {
                modulo_uint(poly, poly_coeff_uint64_count, modulus, coeffmodedptr, pool);
                if (is_greater_than_or_equal_uint_uint(
                    coeffmodedptr, modulusthresholdptr, modulus_uint64_count))
                {
        ...
        void poly_eval_poly_polymod_coeffmod(const uint64_t *poly_to_eval, const uint64_t *value, 
            const PolyModulus &poly_modulus, const Modulus &modulus, uint64_t *result, MemoryPool &pool)
        {
            size_t modulus_uint64_count = modulus.uint64_count();
            size_t poly_modulus_coeff_count = poly_modulus.coeff_count();
#ifdef SEAL_DEBUG
            size_t poly_modulus_coeff_uint64_count = poly_modulus.coeff_uint64_count();
            ...
            auto temp1(allocate_poly(poly_modulus_coeff_count, modulus_uint64_count, pool));
            auto temp2(allocate_zero_poly(poly_modulus_coeff_count, modulus_uint64_count, pool));
            uint64_t *productptr = temp1.get();
            uint64_t *intermediateptr = temp2.get();

            if (poly_modulus.is_fft_modulus())
            {
                for (size_t coeff_index = poly_modulus_coeff_count; coeff_index--; )
                {
                    nussbaumer_multiply_poly_poly_coeffmod(intermediateptr, value, 
                        poly_modulus.coeff_count_power_of_two(), modulus, productptr, pool);
            ...
            else
            {
                for (size_t coeff_index = poly_modulus_coeff_count; coeff_index--; )
                {
                    nonfft_multiply_poly_poly_polymod_coeffmod(intermediateptr, value, poly_modulus, 
                        modulus, productptr, pool);
                    const uint64_t *curr_coeff = get_poly_coeff(poly_to_eval, coeff_index, modulus_uint64_count);
                    add_uint_uint_mod(productptr, curr_coeff, modulus.get(), modulus_uint64_count, productptr);
                    swap(productptr, intermediateptr);
                }
        ...
        void poly_eval_uint_mod(const uint64_t *poly_to_eval, size_t poly_to_eval_coeff_count, 
            const uint64_t *value, const Modulus &modulus, uint64_t *result, MemoryPool &pool)
        {
            size_t modulus_uint64_count = modulus.uint64_count();
#ifdef SEAL_DEBUG
            if (poly_to_eval == nullptr)
...
#endif
            // Evaluate poly at value using Horner's method
            auto temp1(allocate_uint(modulus_uint64_count, pool));
            auto temp2(allocate_zero_uint(modulus_uint64_count, pool));
            uint64_t *productptr = temp1.get();
            uint64_t *intermediateptr = temp2.get();

            for (size_t coeff_index = poly_to_eval_coeff_count; coeff_index--; )
            {
        ...
        void exponentiate_poly_polymod_coeffmod(const std::uint64_t *poly, 
            const uint64_t *exponent, size_t exponent_uint64_count,
            const PolyModulus &poly_modulus, const Modulus &coeff_modulus, 
            std::uint64_t *result, MemoryPool &pool)
        {
            size_t coeff_modulus_uint64_count = coeff_modulus.uint64_count();
            size_t poly_modulus_coeff_count = poly_modulus.coeff_count();
#ifdef SEAL_DEBUG
            size_t poly_modulus_coeff_uint64_count = poly_modulus.coeff_uint64_count();
            if (poly == nullptr)
            ...
            auto big_alloc(allocate_poly(poly_modulus_coeff_count + 
                poly_modulus_coeff_count + poly_modulus_coeff_count, coeff_modulus_uint64_count, pool));

            uint64_t *powerptr = big_alloc.get();
            uint64_t *productptr = get_poly_coeff(
                powerptr, poly_modulus_coeff_count, coeff_modulus_uint64_count);
            uint64_t *intermediateptr = get_poly_coeff(
                productptr, poly_modulus_coeff_count, coeff_modulus_uint64_count);
                ...
                while (true)
                {
                    if ((*exponent_copy.get() % 2) == 1)
                    {
                        nussbaumer_multiply_poly_poly_coeffmod(
                            powerptr, intermediateptr, poly_modulus.coeff_count_power_of_two(), 
                            coeff_modulus, productptr, pool);
                        swap(productptr, intermediateptr);
                    }
                    ...
                    nussbaumer_multiply_poly_poly_coeffmod(
                        powerptr, powerptr, poly_modulus.coeff_count_power_of_two(), 
                        coeff_modulus, productptr, pool);
                ...
                while (true)
                {
                    if ((*exponent_copy.get() % 2) == 1)
                    {
                        nonfft_multiply_poly_poly_polymod_coeffmod(
                            powerptr, intermediateptr, poly_modulus, coeff_modulus, productptr, pool);
                        swap(productptr, intermediateptr);
                    }
                    right_shift_uint(exponent_copy.get(), 1, exponent_uint64_count, exponent_copy.get());
                    if (is_zero_uint(exponent_copy.get(), exponent_uint64_count))
                    ...
                    nonfft_multiply_poly_poly_polymod_coeffmod(
                        powerptr, powerptr, poly_modulus, coeff_modulus, productptr, pool);
                    swap(productptr, powerptr);
                }
            }
