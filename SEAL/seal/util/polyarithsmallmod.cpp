...
namespace seal
{
    namespace util
    {
        void multiply_poly_poly_coeffmod(const uint64_t *operand1, size_t operand1_coeff_count, 
            ...
            const SmallModulus &modulus, size_t result_coeff_count, uint64_t *result)
        {
#ifdef SEAL_DEBUG
        ...
        void multiply_poly_poly_coeffmod(const uint64_t *operand1, const uint64_t *operand2, 
            size_t coeff_count, const SmallModulus &modulus, uint64_t *result)
        {
#ifdef SEAL_DEBUG
        ...
        void divide_poly_poly_coeffmod_inplace(uint64_t *numerator, const uint64_t *denominator, 
            size_t coeff_count, const SmallModulus &modulus, uint64_t *quotient, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            ...
            auto big_alloc(allocate_uint(
                coeff_uint64_count + intermediate_uint64_count + intermediate_uint64_count +
                    7 * coeff_uint64_count, pool));
            ...
            const PolyModulus &poly_modulus, const SmallModulus &modulus)
        {
            ...
            size_t value_coeffs = value_coeff_count;
            while (value_coeffs && value[value_coeffs - 1] == 0)
            {
                value_coeffs--;
            }
            ...
            if (poly_modulus.is_one_zero_one())
            {
                ...
                while (value_coeffs >= poly_modulus_coeff_count)
                {
                    ...
                    if (*leading_value_coeff != 0)
                    {
                        ...
                        value[poly_modulus_shift] = sub_uint_uint_mod(
                            value[poly_modulus_shift], *leading_value_coeff, modulus);
            ...
            while (value_coeffs >= poly_modulus_coeff_count)
            {
                ...
                if (leading_value_coeff != 0)
                {
                    ...
                    temp_quotient = multiply_uint_uint_mod(
                        monic_poly_modulus_scalar, leading_value_coeff, modulus);
            ...
            size_t coeff_count, const SmallModulus &modulus, uint64_t *result, MemoryPool &pool)
        {
            ...
            if (get_significant_coeff_count_poly(
                operand, coeff_count, modulus.uint64_count()) >= 
                    get_significant_coeff_count_poly(poly_modulus, coeff_count, modulus.uint64_count()))
            {
            ...
            while (true)
            {
                ...
                while (numerator_coeffs >= denominator_coeffs)
                {
                    ...
                    if (!is_zero_uint(leading_numerator_coeff, coeff_uint64_count))
                    {
                        ...
                        *temp_quotient.get() = multiply_uint_uint_mod(
                            monic_denominator_scalar, *leading_numerator_coeff, modulus);
                            ...
                            denominator_coeff_index++)
                        {
                            ...
                            const uint64_t *denominator_coeff = get_poly_coeff(
                                denominator, denominator_coeff_index, coeff_uint64_count);
                            *subtrahend.get() = multiply_uint_uint_mod(
                                *temp_quotient.get(), *denominator_coeff, modulus);
        ...
        void exponentiate_poly_polymod_coeffmod(const uint64_t *operand, 
            ...
            uint64_t *result, MemoryPool &pool)
        {
            ...
            if (is_zero_poly(poly_modulus.get(), poly_modulus_coeff_count, 
                poly_modulus_coeff_uint64_count))
            {
                throw invalid_argument("poly_modulus");
            }
            if (result == nullptr)
            ...
            auto power(allocate_uint(poly_modulus_coeff_count, pool));
            auto product(allocate_uint(poly_modulus_coeff_count, pool));
            auto intermediate(allocate_uint(poly_modulus_coeff_count, pool));
                ...
                while (true)
                {
                    if (*exponent_copy.get() & 1)
                    {
                        nonfft_multiply_poly_poly_polymod_coeffmod(
                            powerptr, intermediateptr, poly_modulus, modulus, productptr, pool);
                        swap(productptr, intermediateptr);
                    }
                    right_shift_uint(exponent_copy.get(), 1, exponent_uint64_count, exponent_copy.get());
                    if (is_zero_uint(exponent_copy.get(), exponent_uint64_count))
                    ...
                    nonfft_multiply_poly_poly_polymod_coeffmod(
                        powerptr, powerptr, poly_modulus, modulus, productptr, pool);
                    swap(productptr, powerptr);
                }
            }
