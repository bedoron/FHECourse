...
namespace seal
{
    namespace util
    {
        inline void modulo_poly_coeffs(const std::uint64_t *poly, std::size_t coeff_count, 
        ...
        inline void add_bigpolyarray_coeffmod(const std::uint64_t *array1, 
            const std::uint64_t *array2, int count, std::size_t coeff_count, 
            const SmallModulus &modulus, std::uint64_t *result)
        {
            ...
            std::uint64_t galois_elt, const SmallModulus &modulus, std::uint64_t *result)
        {
            ...
            if (!(galois_elt & 1) || (galois_elt >= 2 * (1ULL << coeff_count_power)))
            {
...
#endif
            std::size_t coeff_count = size_t(1) << coeff_count_power;
            std::uint64_t m_minus_one = 2 * coeff_count - 1;
            for (std::size_t i = 0; i < coeff_count; i++)
            {
                std::uint64_t reversed = reverse_bits(i, coeff_count_power);
                std::uint64_t index_raw = galois_elt * (2 * reversed + 1);
                index_raw &= m_minus_one;
                std::uint64_t index = reverse_bits((index_raw - 1) >> 1, coeff_count_power);
            ...
            const SmallModulus &modulus, std::uint64_t *result, MemoryPool &pool)
        {
            ...
            std::size_t coeff_count = poly_modulus.coeff_count();
            std::size_t intermediate_coeff_count = coeff_count * 2 - 1;
            auto intermediate(allocate_uint(intermediate_coeff_count, pool));
            ...
            const SmallModulus &modulus, std::uint64_t *result)
        {
            ...
            // Calculate normal product.
            std::size_t coeff_count = poly_modulus.coeff_count();
            std::size_t result_coeff_count = coeff_count * 2 - 1;
            ...
            std::uint64_t *result)
        {
            ...
            std::uint64_t index;
            for (std::size_t i = 0; i < coeff_count; i++, operand++, index_raw++)
            {
