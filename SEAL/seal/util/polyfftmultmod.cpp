...
namespace seal
{
    namespace util
    {
        ...
        void nussbaumer_multiply_poly_poly_coeffmod(const uint64_t *operand1, 
            const uint64_t *operand2, int coeff_count_power, const Modulus &modulus, 
        ...
        { 
            ...
            size_t product_coeff_uint64_count = static_cast<size_t>(
                divide_round_up(2 * coeff_bit_count + coeff_count_power + 1, bits_per_uint64)); 
            int sum_bit_count = 1 + coeff_bit_count + coeff_count_power;
            size_t sum_uint64_count = static_cast<size_t>(divide_round_up(sum_bit_count, bits_per_uint64)); 

            auto intermediate(allocate_poly(coeff_count, product_coeff_uint64_count, pool));
            nussbaumer_multiply_poly_poly(operand1, operand2, coeff_count_power, 
                coeff_uint64_count, sum_uint64_count, product_coeff_uint64_count, intermediate.get(), pool);
        ...
        void ntt_multiply_poly_poly(const uint64_t *operand1, const uint64_t *operand2, 
            const NTTTables &tables, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (operand1 == nullptr)
            {
...
#endif
            auto copy_operand1(allocate_poly(tables.coeff_count(), tables.coeff_uint64_count(), pool)); 
            set_poly_poly(operand1, tables.coeff_count(), tables.coeff_uint64_count(), copy_operand1.get()); 
            auto copy_operand2(allocate_poly(tables.coeff_count(), tables.coeff_uint64_count(), pool));
        ...
        */
        void ntt_double_multiply_poly_nttpoly(const uint64_t *operand1, const uint64_t *operand2, 
            const uint64_t *operand3,  const NTTTables &tables, uint64_t *result1, 
            uint64_t *result2,  MemoryPool &pool)
        {
            ...
            set_poly_poly(operand1, tables.coeff_count(), tables.coeff_uint64_count(), copy_operand1.get());
            ntt_negacyclic_harvey(copy_operand1.get(), tables, pool);
            dyadic_product_coeffmod(copy_operand1.get(), operand2, tables.coeff_count(), 
                tables.modulus(), result1, pool);
            dyadic_product_coeffmod(copy_operand1.get(), operand3, tables.coeff_count(), 
                tables.modulus(), result2, pool);
        ...
        void ntt_dot_product_bigpolyarray_nttbigpolyarray(const uint64_t *array1, 
            const uint64_t *array2, size_t count, size_t array_poly_uint64_count, 
            const NTTTables &tables, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (array1 == nullptr)
            ...
            size_t coeff_uint64_count = static_cast<size_t>(divide_round_up(coeff_bit_count, bits_per_uint64));
            ...
            // Create copies
            auto temp(allocate_poly(coeff_count, coeff_uint64_count, pool));
            auto copy_operand1(allocate_poly(coeff_count, coeff_uint64_count, pool));
            ...
            for (size_t i = 0; i < count; i++)
            {
                ...
                dyadic_product_coeffmod(copy_operand1.get(), current_array2, coeff_count, 
                    tables.modulus(), temp.get(), pool);
                add_poly_poly_coeffmod(result, temp.get(), coeff_count, tables.modulus().get(), 
        ...
        void ntt_double_dot_product_bigpolyarray_nttbigpolyarrays(const uint64_t *array1, 
            const uint64_t *array2, const uint64_t *array3, size_t count, size_t array_poly_uint64_count, 
            const NTTTables &tables, uint64_t *result1, uint64_t *result2, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (array1 == nullptr)
            {
            ...
            const Modulus &modulus, uint64_t *result, MemoryPool &pool)
        {
            ...
            int coeff_bit_count = modulus.significant_bit_count();
            size_t coeff_uint64_count = static_cast<size_t>(divide_round_up(coeff_bit_count, bits_per_uint64));
            size_t poly_ptr_increment = coeff_count * coeff_uint64_count;
            ...
            for (size_t i = 0; i < count; ++i)
            {
                ...
                add_poly_poly_coeffmod(result, temp.get(), coeff_count, modulus.get(), 
                    coeff_uint64_count, result);
