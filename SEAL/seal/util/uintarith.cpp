...
namespace seal
{
    namespace util
    {
            ...
            uint64_t *quotient, MemoryPool &pool)
        {
            ...
            uint64_count = static_cast<size_t>(divide_round_up(numerator_bits, bits_per_uint64));
            ...
            auto alloc_anchor(allocate_uint(2LL * uint64_count, pool));
        ...
        void divide_uint_uint_inplace(uint64_t *numerator, 
            const Modulus &denominator, size_t uint64_count, 
            uint64_t *quotient, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (numerator == nullptr && uint64_count > 0)
            ...
            size_t denominator_uint64_count = denominator.uint64_count();
            ...
            uint64_t denominator, uint64_t *quotient)
        {
            ...
            left_shift_uint(shifted_denominator, denominator_shift, uint64_count, shifted_denominator);
            denominator_bits += denominator_shift;
