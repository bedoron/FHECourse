...
namespace seal
{
    namespace util
    {
        template<typename T, typename S, typename = is_uint64_t<T, S>>
            ...
            std::size_t uint64_count, std::uint64_t *result)
        {
...
#endif
            std::size_t uint64_shift_amount = static_cast<std::size_t>(shift_amount / bits_per_uint64);
            int bit_shift_amount = shift_amount - static_cast<int>(uint64_shift_amount) * bits_per_uint64;
            ...
            {
                for (std::size_t i = uint64_count - 1; i >= uint64_shift_amount + 1; i--)
                {
        ...
        void divide_uint_uint_inplace(std::uint64_t *numerator, 
            const Modulus &denominator, std::size_t uint64_count, 
            std::uint64_t *quotient, MemoryPool &pool);
