...
namespace seal
{
    namespace util
    {
        void modulo_uint_inplace(uint64_t *value, size_t value_uint64_count, 
            const Modulus &modulus, MemoryPool &pool)
        {
            ...
            size_t uint64_count = static_cast<size_t>(divide_round_up(value_bits, bits_per_uint64));
            ...
            auto shifted(allocate_uint(uint64_count, pool));
            ...
            if (invmodulus != nullptr)
            {
                ...
                auto product(allocate_uint(uint64_count, pool));
            ...
            auto difference(allocate_uint(uint64_count, pool));
        ...
        bool try_invert_uint_mod(const uint64_t *operand, const uint64_t *modulus, 
            size_t uint64_count, uint64_t *result, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (operand == nullptr)
            {
        ...
        bool is_primitive_root(const uint64_t *root, uint64_t degree, 
            const Modulus &prime_modulus, MemoryPool &pool)
        {
            size_t uint64_count = prime_modulus.uint64_count();
#ifdef SEAL_DEBUG
            if (root == nullptr)
            {
            ...
            auto power(allocate_uint(uint64_count, pool));
            degree >>= 1;
        ...
        bool try_primitive_root(uint64_t degree, const Modulus &prime_modulus, 
            uint64_t *destination, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            ...
            auto alloc_anchor(allocate_uint(3 * uint64_count, pool));
            ...
            // size_entire_group must be zero now, or otherwise the primitive root does not 
            ...
            do
            {
                ...
                exponentiate_uint_mod(destination, size_quotient_group, uint64_count, 
                    prime_modulus, destination, pool);
            }
            while(!is_primitive_root(destination, degree, prime_modulus, pool) && 
                attempt_counter < attempt_counter_max);

            return is_primitive_root(destination, degree, prime_modulus, pool);
        }

        bool try_minimal_primitive_root(uint64_t degree, const Modulus &prime_modulus, 
            uint64_t *destination, MemoryPool &pool)
        {
            ...
            auto generator_sq(allocate_uint(uint64_count, pool));
            multiply_uint_uint_mod(destination, destination, prime_modulus, generator_sq.get(), pool);

            auto current_generator(allocate_uint(uint64_count, pool));
            ...
            for (size_t i = 0; i < degree; i++)
            {
                ...
                auto current_generator_copy(duplicate_uint_if_needed(current_generator.get(), 
                    uint64_count, uint64_count, true, pool));
                multiply_uint_uint_mod(current_generator_copy.get(), generator_sq.get(), 
                    prime_modulus, current_generator.get(), pool);
            }

            return true;
        }

        void exponentiate_uint_mod(const uint64_t *operand, const uint64_t *exponent, 
            size_t exponent_uint64_count, const Modulus &modulus, uint64_t *result, MemoryPool &pool)
        {
            size_t modulus_uint64_count = modulus.uint64_count();
            ...
            auto alloc_anchor(allocate_uint(exponent_uint64_count + 3 * modulus_uint64_count, pool));
