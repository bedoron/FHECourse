...
namespace seal
{
    namespace util
    {
        ...
        inline std::uint64_t barrett_reduce_128(const T *input, const SmallModulus &modulus)
        {
#ifdef SEAL_DEBUG
            if (input == nullptr)
            {
        ...
        inline std::uint64_t modulo_uint(const std::uint64_t *value, 
            std::size_t value_uint64_count, const SmallModulus &modulus, MemoryPool &pool)
        {
#ifdef SEAL_DEBUG
            if (value == nullptr && value_uint64_count > 0)
            {
        ...
        void divide_uint_uint_mod_inplace(uint64_t *numerator, 
            const SmallModulus &modulus, std::size_t uint64_count, uint64_t *quotient, 
            MemoryPool &pool);
    }
}
