...
namespace seal
{
    namespace util
    {
            ...
            int char_count, std::size_t uint64_count, std::uint64_t *result)
        {
            ...
            if (static_cast<std::size_t>(get_hex_string_bit_count(hex_string, char_count)) > 
                uint64_count * static_cast<std::size_t>(bits_per_uint64))
            {
                throw std::invalid_argument("hex_string");
            }
#endif
            ...
            std::size_t uint64_count SEAL_MAYBE_UNUSED, int bit_index)
        {
            ...
            value[static_cast<std::size_t>(uint64_index)] |= (1ULL << sub_bit_index);
        }
        ...
        inline void set_uint_uint(const std::uint64_t *value, 
            std::size_t value_uint64_count, std::size_t result_uint64_count, 
            std::uint64_t *result)
        {
#ifdef SEAL_DEBUG
            ...
            std::size_t uint64_count)
        {
            ...
            if (uint64_count >= std::numeric_limits<int>::max())
            {
                throw std::invalid_argument("uint64_count");
            }
#endif
            operand += uint64_count;
            int long_index = static_cast<int>(uint64_count), local_result = -1;
...
#endif
            operand += uint64_count;
            int long_index = static_cast<int>(uint64_count), local_result = 0;
...
#endif
            if (bit_count == static_cast<int>(uint64_count) * bits_per_uint64)
            {
                return;
            }
            std::size_t uint64_index = static_cast<std::size_t>(bit_count / bits_per_uint64);
            int subbit_index = bit_count - static_cast<int>(uint64_index) * bits_per_uint64;
            operand += uint64_index;
            *operand++ &= (1ULL << subbit_index) - 1;
            for (std::size_t long_index = uint64_index + 1; long_index < uint64_count; 
                long_index++)
            {
                *operand++ = 0;
            }
        ...
        {
#ifdef SEAL_DEBUG
            if (input == nullptr && uint64_count > 0)
            {
