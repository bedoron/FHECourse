...
namespace SEALTest
{
    TEST_CLASS(CKKSEncoderTest)
    {
    public:
        TEST_METHOD(CKKSEncoderEncodeVectorDecodeTest)
        {
            EncryptionParameters parms(scheme_type::CKKS);
            {
                ...
                {
                    std::complex<double> value(static_cast<double>(0.0, 0.0));
                    values[i] = value;
                }
        ...
        TEST_METHOD(CKKSEncoderEncodeSingleDecodeTest)
        {
            EncryptionParameters parms(scheme_type::CKKS);
            {
                uint32_t slots = 16;
