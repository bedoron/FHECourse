    ...
    namespace util
    {
        ...
        public:
            TEST_METHOD(NTTTablesBasicsTest)
            {
                ...
                NTTTables tables;
                auto modulus_anchor(allocate_uint(1, pool));
                modulus_anchor[0] = 37;
                ...
                Assert::AreEqual(tables.coeff_count(), 2ULL);
                Assert::IsTrue(tables.is_generated());
                Assert::AreEqual(tables.coeff_uint64_count(), 1ULL);
                Assert::AreEqual(tables.coeff_count_power(), 1);
            }
            ...
            {   
                ...
                NTTTables tables;
                {
                    ...
                    Modulus modulus(modulus_anchor.get(), 1, pool);
                    tables.generate(coeff_count_power, modulus);
                    Assert::AreEqual(*tables.get_from_root_powers(1), 6ULL);
                ...
                {
                    auto modulus_anchor(allocate_uint(2, pool));
            ...
            TEST_METHOD(NegacyclicNTTTest)
            {
...

                {
                    ...
                    Modulus modulus(modulus_anchor.get(), 1, pool);
                    tables.generate(coeff_count_power, modulus);
                    auto poly(allocate_zero_poly(8, 1, pool));
                    ntt_negacyclic_harvey(poly.get(), tables, pool);
                    for (int i = 0; i < 8; ++i)
                    {
            ...
            TEST_METHOD(InverseNegacyclicNTTTest)
            {
...

                {
                    ...
                    tables.generate(coeff_count_power, modulus);
                    auto poly(allocate_zero_poly(8, 1, pool));
                    auto temp(allocate_zero_poly(8, 1, pool));
