...
namespace SEALTest
{
    namespace util
    {
        ...
        public:
            ...
            {
                MemoryPool &pool = *global_variables::global_memory_pool;
                auto ptr1(allocate_zero_poly(2, 3, pool));
                auto ptr2(allocate_zero_poly(2, 3, pool));
                for (int i = 0; i < 6; ++i)
                {
                    ptr1[i] = static_cast<uint64_t>(i + 1);
            ...
            {
                Assert::AreEqual(0ULL, get_significant_coeff_count_poly(nullptr, 0, 0));

                MemoryPool &pool = *global_variables::global_memory_pool;
                auto ptr(allocate_zero_poly(3, 2, pool));
                Assert::AreEqual(0ULL, get_significant_coeff_count_poly(ptr.get(), 3, 2));
                ptr[0] = 1;
                Assert::AreEqual(1ULL, get_significant_coeff_count_poly(ptr.get(), 3, 2));
                ptr[1] = 1;
                Assert::AreEqual(1ULL, get_significant_coeff_count_poly(ptr.get(), 3, 2));
                ptr[4] = 1;
                Assert::AreEqual(3ULL, get_significant_coeff_count_poly(ptr.get(), 3, 2));
                ptr[4] = 0;
                ptr[5] = 1;
                Assert::AreEqual(3ULL, get_significant_coeff_count_poly(ptr.get(), 3, 2));
            }
            ...
            TEST_METHOD(DuplicateBigPolyArrayIfNeeded)
            {
                ...
                auto ptr = duplicate_bigpolyarray_if_needed(poly.get(), 3, 2, 1, 3, 2, 1, false, pool);
                Assert::IsTrue(ptr.get() == poly.get());
